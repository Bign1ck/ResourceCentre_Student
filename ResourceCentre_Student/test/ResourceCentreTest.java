...
public class ResourceCentreTest {
	...
	public void testDoLoanChromebook() {
		...
		ResourceCentre.addChromebook(chromebookList, cb1);
		boolean isLoanedOut = ResourceCentre.doLoanChromebook(chromebookList, cb1.getAssetTag(), "8-8-2020");
		assertTrue("Test if CB0011 is successfully loaned out.", isLoanedOut);
		assertFalse("Test that CB0011 is no longer available.", chromebookList.get(0).getIsAvailable());
		assertEquals("Test that the due date is set correctly.", "8-8-2020", chromebookList.get(0).getDueDate());

		// Test Case 2: Loan an unavailable item
		cb2.setIsAvailable(false);
		ResourceCentre.addChromebook(chromebookList, cb2);
		assertFalse("Test that CB0012 is not available to be loaned out.",
				ResourceCentre.doLoanChromebook(chromebookList, cb2.getAssetTag(), "8-8-2020"));

		// Test Case 3: Item not found
		assertFalse("Test that loaning an item that does not exist fails.",
				ResourceCentre.doLoanChromebook(chromebookList, "CB0013", "8-8-2020"));

		// Test Case 4: Missing details
		ResourceCentre.addChromebook(chromebookList, cb3);
		assertFalse("Test that loaning an item with missing details fails.",
				ResourceCentre.doLoanChromebook(chromebookList, cb3.getAssetTag(), ""));
	}
